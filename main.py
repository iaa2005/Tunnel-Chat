...
genBtn = tk.Button(text="Info", width=6, bd=0)
genBtn.place(x=325, y=137)
...
EntryChatID = tk.Entry(bd=0, highlightthickness=2, highlightbackground="white", width=21)
EntryChatID.place(x=15, y=137)

connectBtn = tk.Button(text="Connect", width=10)
...
def connect():
    ...
    if CHAT_ID == "":
        ...
        ChatIDText.configure(text=CHAT_ID)
    input_user.set("")
    connectBtn.configure(text="Connected")

connectBtn.configure(command=connect)
connectBtn.place(x=220, y=137)

input_user = tk.StringVar()
input_field = tk.Entry(text=input_user, highlightthickness=2, highlightbackground="white", bd=0, width=30)
...
fileText = tk.Label(text="", bg=DARK_GREY, fg=WHITE_03,
                       font=tkFont.Font(family="Helvetica", size=10, weight="normal"))
fileText.place(x=10, y=615)
...
def choose_file():
    ...
    if text_type_files != {}:
        ...
        CAN_SEND_FILE = True
        fileText.configure(text=filenames_window)
...
sendBtn = tk.Button(text="Send", width=9, command=send_text, bd=0)
sendBtn.place(x=300, y=587)

chooseFile = tk.Button(text="Choose file", width=9, command=choose_file, bd=0)
chooseFile.place(x=design["chooseFile-x"], y=design["chooseFile-y"])

sendFileBtn = tk.Button(text="Send file", width=9, command=send_files, bd=0)
sendFileBtn.place(x=design["sendFileBtn-x"], y=design["sendFileBtn-y"])
